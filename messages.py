...
def answer_request(choice, other_id):
    ...
    convo_id = db.session.execute(sql, {'user_id':session['user_id'], 'other_id':other_id}).fetchone().id
...
def send_message(content, receiver_id, convo_id):
    ...
    db.session.execute(sql, {'content':content, 'receiver':receiver_id, 'convo_id':convo_id, 'sender':session['user_id']})
    db.session.commit()

def show_convos():
    sql = text('SELECT DISTINCT displayname FROM users u, messages m WHERE \
                (u.id=m.receiver_id AND m.sender_id=:user_id) OR (u.id=m.sender_id AND m.receiver_id=:user_id)')
